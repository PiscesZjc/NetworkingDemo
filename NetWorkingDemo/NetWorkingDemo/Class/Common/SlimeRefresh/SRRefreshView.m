...
- (id)initWithHeight:(CGFloat)height
{
    ...
    if (self) {
        _slime = [[SRSlimeView alloc] initWithFrame:
                  CGRectMake(0.0f, 0.0f, SCREEN_WIDTH, frame.size.height)];
        _slime.startPoint = CGPointMake(SCREEN_WIDTH / 2, height / 2);
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (_slime.state == SRSlimeStateNormal) {
        _slime.frame = CGRectMake(0.0f, 0.0f, SCREEN_WIDTH, frame.size.height);
        _slime.startPoint = CGPointMake(SCREEN_WIDTH / 2, _dragingHeight / 2);
    }
    _refleshView.center = _slime.startPoint;
    _activityIndicatorView.center = _slime.startPoint;
}
